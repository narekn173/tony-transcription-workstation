...
| Basic Pitch | MIDI or note-events CSV notes, possibly polyphonic | `NoteModel` + `NoteLayer` or `FlexiNoteLayer` only after converter proof and explicit warnings for unresolved polyphony | Multiple-layer/voice strategy if needed | Blind monophonic flattening or treating raw CLI artifacts as Tony import | Real Basic Pitch output, converter-created `UnifiedResult`, overlap policy, real layer import, edit/save/load/export proof. CODEX-101 proves Basic Pitch-shaped `result.json` to real note-layer import in manual/test-only scope, not production UI integration. |
...
Use this matrix before Basic Pitch save/load/export, debug UI, selected-region, or production workflow work. Any future importer task should cite the exact row it is implementing and the proof gates it intends to satisfy.
