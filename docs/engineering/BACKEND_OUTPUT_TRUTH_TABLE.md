...
| Basic Pitch | Repository-verified. Basic Pitch is an audio-to-MIDI AMT system with multipitch/polyphonic support and pitch-bend-capable MIDI output. | Compatible audio file, usually adapter-prepared WAV/audio path. | Notes/MIDI-like events, pitch bends, model outputs, possible confidence/velocity-like fields depending on output path. | Treat as potentially polyphonic. Never force overlapping notes into one monophonic Tony interpretation without an explicit policy. | Notes may map to `NoteModel` + `NoteLayer` only after polyphony policy. `NoteModel::FLEXI_NOTE` + `FlexiNoteLayer` only if flexi semantics are intentionally chosen. Pitch bends require a separate bend/deviation representation or explicit warning. | Editable only after real Tony note layer proof. Import must use `Document`/`LayerFactory`/`CommandHistory`. Save/load/export proof is required for any feature claim. | Flattened polyphony, lost pitch bends, lost confidence/velocity, ambiguous channels/voices, false "completed" state from generated files alone. | Real Basic Pitch run on real audio; real output file; valid `UnifiedResult`; polyphony policy applied; notes imported into real Tony layer; bends/confidence preserved or explicitly warned; edit/save/load/export proof if claimed. |
